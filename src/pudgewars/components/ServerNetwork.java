...
public class ServerNetwork extends Network{
	...
	public ServerNetwork(List<ClientNode> clients){
		...
		for (int x = 0; x < clients.size(); x++) serverConn.add(clients.get(x).getConnection());
	...
	public void sendPudgeEntities(List<Entity> entities){
		for (int x = 0; x < serverConn.size(); x++) {
			for (int y = 0; y < entities.size(); y++) {
				...
				serverConn.get(x).sendMessage("GENERATE " + entities.get(y).getX() + " " + entities.get(y).getY() + " " + entities.get(y).team + " " + controllable);
			}
